...
function StreetsScreen({ sector }: { sector: string }) {
  ...
  if (error) return (
    <div className={styles.loading}><p style={{color:'var(--red)'}}>{error}</p></div>
  )

  const sectorData = data?.sectors?.[sector] ?? {}
  const streets = Object.keys(sectorData).sort((a, b) => {
  ...
  return (
    <>
      ...
      <div className={styles.content}>
        <p className={styles.sectionLabel}>{filtered.length} Streets in {sector}</p>
        {filtered.length === 0
          ? <EmptyState message="No streets match your search" />
          : filtered.map(street => {
              const count = sectorData[street]?.length ?? 0
              return (
                <Link
                  key={street}
...
export default function StreetsPage({ params }: { params: { sector: string } }) {
  const decoded = decodeURIComponent(params.sector)
  return (
    <DataProvider>
